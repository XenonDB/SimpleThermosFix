 ...
 * 例如，把一個沒有實作IClassTransformer的類別，定義了一個有實作內容的public byte[] transform(String
 ...
 * 這導致寵物無法簡單的新增InventoryHolder
 * getOwner()來適應修改後的類別，因為編譯器並不允許重載相同方法名、相同參數但返回類型不同的方法(儘管這在執行時期是可行的，因為執行時呼叫的方法簽署包含了返回類型，可以被辨認是不同的方法)
...
public class ASMMixinsIInventory implements IClassTransformer {
	...
	public byte[] transform(String name, String transformedName, byte[] data) {
		...
		// 令其繼承有定義預設getOwner()實做的interface來「將getOwner()新增預設實作」
